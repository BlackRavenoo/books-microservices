<script lang="ts">
    ...
    async function handleSubmit() {
        ...
        try {
            await createAuthor(coverFile, { name });
            success = true;
        } catch (err: unknown) {
            error = err instanceof Error ? err.message : 'Произошла ошибка при создании автора';
        } finally {
            isSubmitting = false;
        }
    }
...
<div class="container">
    <h1 class="form-title">Создание нового автора</h1>
    
    {#if error}
        <div class="error-message">{error}</div>
    {:else if success}
        <div class="success-message">
            <h2>Автор успешно создан!</h2>
    ...
    {:else}
        <form on:submit|preventDefault={handleSubmit} class="author-form">
            ...
            <div class="form-actions">
                ...
                >
                    {isSubmitting ? 'Создание...' : 'Создать автора'}
                </button>
            </div>
        </form>
    {/if}
