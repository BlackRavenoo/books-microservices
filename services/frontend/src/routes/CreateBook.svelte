<script lang="ts">
    ...
    import { createBook, fetchConstants, search } from '../api';
    import type { CreateBookFields } from '../types';
    ...
    onMount(async () => {
        ...
        try {
            ...
            error = err instanceof Error ? err.message : 'An error occurred while loading form data';
        } finally {
            isLoading = false;
        }
    });
    ...
    async function handleSubmit() {
        ...
        if (!coverFile) {
            error = 'Обложка книги обязательна';
            return;
        }
        ...
        try {
            ...
            await createBook(coverFile, fields);
            ...
            error = err instanceof Error ? err.message : 'An error occurred while creating the book';
        } finally {
            isSubmitting = false;
        }
    }
...
<div class="container">
    <h1 class="form-title">Добавление новой книги</h1>
    
    {#if isLoading}
        <div class="loading">Загрузка данных формы...</div>
    {:else if error}
    ...
    {:else}
        <form on:submit|preventDefault={handleSubmit} class="book-form">
            ...
            <div class="form-actions">
                ...
                >
                    {isSubmitting ? 'Создание...' : 'Создать книгу'}
                </button>
            </div>
        </form>
    {/if}
